...
(function() {
  ...
  if (Meteor.isClient) {
    ...
    // reactive data source for use in Tracker.autoruns
    // in cases where we need to rely on a flipper before the sub has come down
    Flipper.ready = function () {
      return (subscription && subscription.ready());
    };

    Meteor.startup(function () {
      subscription = Meteor.subscribe('flippers');
    });
